#include <Servo.h>
 ...
 int16_t gx, gy, gz;
  ...
  int aci1=135,aci2=60,aci3=135,aci4=90;
...
void loop(){
  ...
  ekle=map(ay,17000,-17000,-90,90);
            if(ekle>average or ekle<-average){aci1+=ekle/10;}
              aci1=duzelt(aci1,0,180);
  ekle=map(ax,17000,-17000,-90,90);
            if(ekle>average or ekle<-average){aci2+=ekle/10;}
              aci2=duzelt(aci2,0,180);
  
  
  Serial.println(String(ay)+" --- "+String(ax));dly(5);
